...
/*
 *enum cam_vfe_bus_ver3_stored_irq_masks: define buf_done/rup mask parameters
 */
	...
	struct cam_vfe_bus_ver3_vfe_out_data *rsrc_data)
{
	...
	if ((rsrc_data->out_type == CAM_VFE_BUS_VER3_VFE_OUT_FD) &&
		...
		num_wm = 1;
	else if ((rsrc_data->num_wm == 3) && ((rsrc_data->format == CAM_FORMAT_NV12) ||
			(rsrc_data->format ==  CAM_FORMAT_NV21)))
		num_wm = 2;
	else
		num_wm = rsrc_data->num_wm;
...
static enum cam_vfe_bus_ver3_packer_format
	cam_vfe_bus_ver3_get_packer_fmt(struct cam_vfe_bus_ver3_wm_resource_data *wm_data)
{

	if ((wm_data->num_planes == 3) && ((wm_data->format == CAM_FORMAT_NV12) ||
			(wm_data->format ==  CAM_FORMAT_NV21)))
	...
	case CAM_FORMAT_NV21:
		if ((wm_data->index == 1) || (wm_data->index == 3) ||  (wm_data->index == 5))
	...
	struct cam_vfe_bus_ver3_priv *bus_priv)
{
	...
	for (i = 0; i < bus_priv->num_out; i++) {
		...
		num_wm = cam_vfe_bus_ver3_get_num_wm(out_rsrc_data);
		for (j = 0; j < num_wm; j++) {
	...
	struct cam_vfe_bus_ver3_wm_resource_data  *rsrc_data)
{
	...
	case CAM_FORMAT_PLAIN16_16:
		...
		if (rsrc_data->use_wm_pack) {
			rsrc_data->pack_fmt = cam_vfe_bus_ver3_get_packer_fmt(rsrc_data);
			/* LSB aligned */
	...
	uint32_t            *comp_grp_id)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%u comp_grp:%d Bus IRQ status_0: 0x%X rc:%d comp_done_mask: 0x%x",
		rsrc_data->common_data->core_index, rsrc_data->comp_grp_type,
		status_0, rc, rsrc_data->comp_done_mask);
	...
	uint32_t args_size, bool update_only)
{
	...
	uint32_t       out_port_res_type, num_wm = 0;
	...
	num_wm = cam_vfe_bus_ver3_get_num_wm(rsrc_data);
	...
	rsrc_data->per_port_en = is_per_port_acquire;
	...
	uint32_t args_size)
{
	...
	struct cam_isp_resource_node          *vfe_out = NULL;
	struct cam_vfe_bus_ver3_vfe_out_data  *rsrc_data = NULL;
	uint32_t                               secure_caps = 0, num_wm = 0;
	...
	for (i = 0; i < CAM_VFE_BUS_VER3_MAX_STORED_MASKS; i++) {
		for (j = 0; j < CAM_VFE_BUS_VER3_IRQ_MAX; j++)
			rsrc_data->stored_irq_masks[i][j] = 0;
	}
	...
	struct cam_isp_resource_node          *vfe_out)
{
	...
	uint32_t source_group = 0, num_wm = 0;
	...
	CAM_DBG(CAM_ISP, "res_id :%d bus_irq_reg_mask[0] :0x%x bus_irq_reg_mask[1] :0x%x", vfe_out->res_id,
		rsrc_data->stored_irq_masks[CAM_VFE_BUS_VER3_BUF_DONE_MASK][CAM_VFE_BUS_VER3_IRQ_REG0],
		rsrc_data->stored_irq_masks[CAM_VFE_BUS_VER3_BUF_DONE_MASK][CAM_VFE_BUS_VER3_IRQ_REG1]);

	if (vfe_out->is_per_port_start ||
		(!vfe_out->linked && rsrc_data->per_port_en)) {
	...
	struct cam_vfe_bus_ver3_priv              *bus_priv)
{
	...
	uint32_t  outmap_index = CAM_VFE_BUS_VER3_VFE_OUT_MAX, num_wm = 0;
	...
	uint32_t arg_size)
{
	...
	struct cam_vfe_bus_ver3_wm_resource_data *wm_data = NULL;
	struct cam_vfe_bus_ver3_reg_offset_ubwc_client *ubwc_regs;
	uint32_t i, val, iova_addr, iova_offset, stride, num_wm = 0;
	dma_addr_t iova;
	...
	uint32_t arg_size)
{
	...
	num_wm = cam_vfe_bus_ver3_get_num_wm(vfe_out_data);
	if ((update_buf->wm_update->num_buf != num_wm) &&
		(!(update_buf->use_scratch_cfg))) {
		CAM_ERR(CAM_ISP,
			"VFE:%u Failed! Invalid number buffers:%d required:%d",
			bus_priv->common_data.core_index, update_buf->wm_update->num_buf, num_wm);
		return -EINVAL;
	}
	...
	uint32_t arg_size)
{
	...
	uint32_t outport_id, ports_plane_idx, i, num_wm = 0;
	...
	void                                        *cmd_args)
{
	...
	for (i = 0; i < num_wm; i++) {
		...
		if ((wm_config->packer_format != CAM_FORMAT_BASE) &&
			...
			wm_data->format = wm_config->packer_format;
			packer_fmt = cam_vfe_bus_ver3_get_packer_fmt(wm_data);
...
add_reg_pair:
	...
	if (num_regval_pairs) {
		...
		CAM_DBG(CAM_ISP,
			"VFE:%u No reg val pairs. num_wms: %u",
			vfe_out_data->common_data->core_index, num_wm);
	...
	uint32_t args_size)
{
	...
	struct cam_isp_resource_node           *rsrc_node = NULL;
	struct cam_vfe_bus_ver3_vfe_out_data   *rsrc_data = NULL;
	uint32_t                                secure_caps = 0, mode, num_wm = 0;
	struct cam_vfe_bus_ver3_comp_grp_acquire_args comp_acq_args = {0};
	uint32_t       outmap_index = CAM_VFE_BUS_VER3_VFE_OUT_MAX;
...
static int cam_vfe_bus_ver3_update_res_comp_grp_state(
	struct cam_vfe_bus_ver3_priv         *ver3_bus_priv,
	struct cam_isp_resource_node           *wm_res,
	bool enable)
{
	...
	if (enable)
		comp_grp_local->res_state = CAM_ISP_RESOURCE_STATE_STREAMING;
	else
		comp_grp_local->res_state = CAM_ISP_RESOURCE_STATE_RESERVED;
...
static int cam_vfe_bus_ver3_enable_irq_vfe_out(void *bus_priv, void *res_irq_mask)
{
	...
	for (i = 0; i < irq_args->num_res; i++) {
		...
		if (!vfe_out->irq_handle && !vfe_out->is_per_port_acquire &&
			...
			num_wm = cam_vfe_bus_ver3_get_num_wm(rsrc_data);
			for (j = 0; j < num_wm; j++) {
				cam_vfe_bus_ver3_update_res_comp_grp_state(bus_priv,
					&rsrc_data->wm_res[j], irq_args->enable_irq);
			}

			if (!irq_args->enable_irq)
				vfe_out->res_state = CAM_ISP_RESOURCE_STATE_RESERVED;
			else
				vfe_out->res_state = CAM_ISP_RESOURCE_STATE_STREAMING;
